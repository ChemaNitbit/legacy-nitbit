...
class ChatMessagesService {
    ...
    async listenAllChats(): Promise<ChatType[]> {
        try {
            ...
            const responseSnapshot = await new Promise<QuerySnapshot>((resolve, reject) => {
            ...
            const iterate = async () => {
                for (const chat of responseSnapshot.docs) {
                    ...
                    if (!!messages.length) {
                        const targetUserData: UserType = await userService.getUserById(chat.data().recipientId);
                        chatsList.push({ id: chat.id, ...chat.data(), messages, targetUserData } as ChatType);
                    }
