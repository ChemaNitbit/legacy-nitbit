...
const MessageTempTab = ({ navigation }: any) => {
    ...
    return (
        <KeyboardAvoidingView style={{ flex: 1, display: 'flex', flexDirection: 'column', paddingHorizontal: 15, backgroundColor: '#fff' }} behavior='height'>

            <View style={{ width: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', gap: 10, minHeight: 50 }}>
            ...
            <View style={{ width: '100%', display: 'flex', flexDirection: 'row', gap: 10, alignItems: 'center', height: 80 }}>
                {
                    userFriends.map((friend: UserType) => {
                        return (
                            <NbAvatar key={friend.id} url={friend?.profilePhoto as string} size={60} />
                        )
                    })
                }
            </View>
            ...
            <View style={{ flex: 10 }}>
                ...
                <FlatList
                    data={chats}
                    renderItem={({ item }) => <NbChatItem chat={item} navigation={navigation}
                    />
                    }
