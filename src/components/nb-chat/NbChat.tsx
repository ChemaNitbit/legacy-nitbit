    ...
    return (
        <View style={{ width: '100%' }}>
            <SafeAreaView style={{
                ...
                <ScrollView contentContainerStyle={[styles.container, { flexGrow: 1, justifyContent: 'flex-end', alignItems: 'flex-end' }]}>

                    {
                        messages?.map((message: MessageType) =>
                            <View key={message.id} style={[styles.messageContainer, {
                                alignSelf: message.senderId === currentUser?.uid ? 'flex-end' : 'flex-start',
                                backgroundColor: message.senderId === currentUser?.uid ? 'blue' : 'green'
                            }]}
                            >
